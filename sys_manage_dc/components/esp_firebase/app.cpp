...
//     // Gắn certificate bundle vào client
//     esp_crt_bundle_attach(FirebaseApp::client);
...
{   
    ...
    ESP_LOGD(FIREBASE_APP_TAG, "HTTP Client Initialized with certificate bundle");
}
...
esp_err_t FirebaseApp::getRefreshToken(bool register_account)
{
    ...
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
        cJSON *root = cJSON_Parse(FirebaseApp::local_response_buffer);
        if (root) {
            cJSON *refresh_token_item = cJSON_GetObjectItem(root, "refreshToken");
            if (cJSON_IsString(refresh_token_item)) {
                FirebaseApp::refresh_token = refresh_token_item->valuestring;
            }
            cJSON_Delete(root);
        }
    ...
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
        ...
        if (root) {
            ...
            if (cJSON_IsString(access_token_item)) {
                FirebaseApp::auth_token = access_token_item->valuestring;
            }
            cJSON_Delete(root);
        }
